<template>
<div class="quickEditorCodeSelector-wrapper">
  <input class="editInput" v-model="inputVal" type="text" @click="searchClick">
  <commonDialog v-if="showHandle" class="filter" :title='placeholder?placeholder:"代码集选择"' appendBody
                :visible.sync="dialogVisible"
                height="400px"
                :save="save" width="600px" :is-draggable="true">
    <div class="codeSelectorTab-item">
      <commonTablistX class="padding" position="top" :options="tabsXData" @change="onTabChange" v-model="checkedX"/>
...
<script>
import codeData from '@/assets/codeData.json'
export default {
name: 'codeSelector',
props: {
  readonly: {                                             //是否可输入
      ...
      if (Array.isArray(this.value) && this.value.length > 0) {
        ...
        this.value.forEach(item => {
          let result = this.data.find((item2) => {
            ...
            return item2[this.props.label] === item;
          });
          if (result) {
        ...
        let result = this.data.filter((item) => {
          ...
          return item[this.props.label] === this.value;
        });
          console.log({result})
        if (result.length > 0) {
...
methods: {
  ...
  getStockData() {
    let localData = JSON.parse(localStorage.getItem(this.stockKey)) || []
  ...
  nodeClick(val, data) {
    ...
    this.checkedVal = data[this.props.label]; // 特意修改
  },
  //节点选中
  checkedChange(data, checkedVal, checkedData) {
    console.log({data, checkedVal, checkedData})
    // this.checkedVal = checkedVal;
    this.checkedVal = checkedData[this.props.label]; // 特意修改
  ...
  save() {
    ...
    // this.$emit("input", this.checkedData.nameCn);
    // this.$emit("save", {
  ...
  setStock({data, value}) {
    ...
    if (localStorage.getItem(this.stockKey)) {
      let historyData = JSON.parse(localStorage.getItem(this.stockKey))
    ...
    if (!stock.map(item => item[this.props.label]).includes(value)) {
...
.quickEditorCodeSelector-wrapper {
