<template>
  <div :class="['common-upload-wrapper',{'iconType':type==='icon'}]">
    ...
    <div class="avatar" v-if="avatar">
      <img class="img" :src="initImg()" onerror="this.src='static/img/defaultAvatar.jpg'" title="点击下载" alt="">
    </div>
    <!-- 单文件1.fileName,2.value -->
    <div class="file file-list" v-if="fileName && hideFile && showFileList">
      <!-- {{initImg()}} -->
      <div class="operate">
        <span v-if="isReview" title="预览" @click="showView()"><i class="iconfont icon-yulan"></i></span>
        <span v-if="isOnRemove" title="删除" @click="removeSingleFile()"><i class="iconfont icon-close"></i></span>
      </div>
      <img v-if="isImage" :src="showImg ? initImg() : imageUrl " onerror="this.src='static/img/defaultAvatar.jpg'"
           title="点击下载" alt="" @click="downLoad()">
      <svg v-else class="svgicon" aria-hidden="true" title="点击下载" @click="downLoad()">
        <use :xlink:href="[`#`+getFileType(fileName)]"></use>
      </svg>
      <div v-if="showFileTitle" class="singerName" :title="fileName">{{ fileName }}</div>
    </div>
    <div v-if="fileName===undefined && !avatar && showFileList">
      <!-- 多文件 1.value-->
      <div :class="['file-list']" v-for="(file,index) in fileList" :key="index">
        <div class="operate">
          <a target="_blank" title="下载" :href="getDownloadUrl(file)"><span><i
              class="iconfont el-icon-download"></i></span></a>
          <span title="删除" @click="removeFile(file,index)"><i class="iconfont icon-close"></i></span>
        </div>
        <div class="preview">
          <img :src="file.url" onerror="this.src='static/img/defaultAvatar.jpg'" title="点击下载" alt=""
               @click="downLoad()">
    ...
    <el-upload class="uploadFile"
               ...
               :on-change="onChange"
    >
      <i v-if="type==='icon'" class="iconType el-icon-plus avatar-uploader-icon"></i>
      <el-button v-if="type==='text'" type="text underline primary">{{ text }}</el-button>
      <el-button v-if="type==='button'" type="primary">{{ text }}</el-button>
	  <span class="el-upload__tip" style="color: red;">禁止上传涉密信息!</span>
      ...
      <div v-if="type==='drag'" class="drag">
        ...
        <div class="el-upload__text">将文件拖到此处，或<em>{{ text }}</em></div>
      </div>
    </el-upload>
...
export default {
  ...
  props: {
    ...
    type: {                                  //显示类型1.button,2.icon,3.text,4.drag
      type: String,
      default: "button"
    },
    showType: {                              //文件列表展示类型1.text,2.picture,3.picture-card
      type: String,
      default: "text"
    },
    showFileList: {                                //是否上传头像
    ...
    viewImg: {                              //新疆多个upload显示文件地址后面要拼接name,例如A135信息集
      type: Boolean,
      default: false,
    },
    //是否删除
  ...
  methods: {
    ...
    onExceed(files, fileList) {
      this.$showMsg("warning", `当前限制选择 ${this.limit} 个文件，本次选择了 ${files.length} 个文件，共选择了 ${files.length + fileList.length} 个文件`)
    },
    //清除文件
    clearFile() {
      console.log("清除文件")
    ...
    initImg() {
      ...
      if (this.loadImg)
        return this.loadImg() + `&code=${this.formatX}`;
      if (this.avatar)
    ...
    showView() {
      ...
      if (this.reviewFile) {
        ...
        if (this.fileName) {
          if (this.viewImg) {
            downLoadUrl = this.$baseUrl + `cadre/download/media?subjectId=${this.value}_${this.name}`;
          } else {
            downLoadUrl = this.$baseUrl + `cadre/download/media?subjectId=${this.value}`;
          }
        } else {
          downLoadUrl = this.$baseUrl + `cadre/photo?empId=${this.value}&code=${this.formatX}`;
        }
    ...
    onBeforeUpload(file) {
      ...
      if (accept) {
        ...
        if (!isMatching) {
          this.$message.warning('上传文件只能是' + msg.substr(0, msg.length - 1) + '格式!');
          this.$store.commit("updatePageLoading", --this.$store.state.pageLoading);
        }
      } else {
        isMatching = true
