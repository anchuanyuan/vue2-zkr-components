<template>
    <el-dialog :id="id" :class="['common-dialog-wrapper',{'fullscreen':fullscreen}, {'notFoot':!showFooter},{'needPadding':padding}]" :top="top" :modal-append-to-body="modelAppendBody" :modal="showModal"
        @open="open" :width="width" :visible.sync="showVisible" :append-to-body="appendBody" :fullscreen="fullscreen" :destroy-on-close="destroyOnClose"
        :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" :before-close="_=>$emit('beforeClose')" v-draggable="isDraggable">
        <div slot="title" class="dialog-title">
            <div class="left-title"><slot name="title">{{title}}</slot></div>
            <span class="right-title iconfont el-icon-full-screen" title="全屏" v-if="showFullScreen&&!isDraggable" @click="changeFull"></span>
            <!-- " @click="fullscreen=!fullscreen" -->
            <span class="right-title iconfont icon-close" title="关闭" @click="closeVisible()" v-if="showClose"></span>
        </div>
        ...
        <div slot="footer" class="dialog-footer" v-if="showFooter">
            ...
            <div class="foot-operate">
                <slot name="footer">
                    ...
                    <slot name="more"></slot>
                    <el-button type="primary" :disabled="disabled" @click="saveForm" v-if="showConfirm">确 定</el-button>
                </slot>
            </div>
        </div>
    </el-dialog>
          ...
          cancel(){
            ...
            saveForm() {
                ...
                if(!ruleResult)
                    return;
                this.disabled = true;
                if (this.save) {//调用外部自己的save方法
	                this.save(()=> {this.disabled = false;});
	                this.disabled = false;
                    return;
                }
                if (this.url || this.updateUrl) {//->有保存或更新路径执行
                    let ajaxOptions = {
