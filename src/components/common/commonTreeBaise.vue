...
<div :id="id" :ref="id" class="common-tree-wrapper">
    <div class="treeHead">
        ...
        <slot name="filter" v-if="filterable">
            <formInput radius class="filterInput" v-model="filterText" suffixIcon="el-icon-search" placeholder="请输入搜索关键字"></formInput>
        </slot>
    </div>
...
<script>
    export default {
        ...
        props: {
            ...
            expandNode:Boolean,                                     //是否点击节点即展开节点
        ...
        methods: {
            ...
            initData(data=this.treeData||[], parent) {
                ...
                data.forEach((item, index) => {
                    ...
                    if(this.props.isFalse && item[this.props.isFalse]) 
                ...
                for (let i=0; i < isFalseArray.length; i++){
                    if(isFalseArray[i][this.props.isFalse]) {
                        ...
                        if (this.selectable){
                            ...
                            return this.nodeClick(isFalseArray[i]);
                        }
                    }
                }
            },
